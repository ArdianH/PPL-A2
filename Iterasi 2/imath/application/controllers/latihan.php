...
class Latihan extends CI_Controller {
	...
	public function processSoal($param = '')
	{
		...
		if($nomorSoal < 10 && $param != 'selesai'){
			...
			if($skor == 100) {
				if($this->latihan_model->getJumlahMedali($this->session->userdata('username'), $idMateri) == 0) {
					$res = $this->latihan_model->addMedali($this->session->userdata('username'), $idMateri);
				}
				$data['medali'] = TRUE;
			}
